...
pub fn run(
    ...
    // --- LOOP ELIMINASI (Conditional - Group-aware) ---
    loop {
        step_count += 1;
        ...
        // Evaluate each included group using conditional LL
        if !included_group_indices.is_empty() {
            for (loc, &g_idx) in included_group_indices.iter().enumerate() {
                ...
                // For multi-column groups, compute conditional LL by zeroing all group betas
                ...
                // Zero out all betas in this group and adjust others
...
fn calculate_step_snapshot(
    ...
    // 4. Model If Term Removed (Conditional Parameter Estimates - Group-aware)
...
// SPSS "Conditional" method: sets β_j = 0 and computes LL with remaining betas adjusted.
