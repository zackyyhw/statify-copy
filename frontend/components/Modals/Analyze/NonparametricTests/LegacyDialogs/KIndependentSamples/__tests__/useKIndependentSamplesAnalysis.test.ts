...
import { useKIndependentSamplesAnalysis } from '../hooks/useKIndependentSamplesAnalysis';
...
jest.mock('../utils/formatters', () => ({
  formatRanksTable: jest.fn(() => ({ headers: [], rows: [] })),
  formatKruskalWallisHTestStatisticsTable: jest.fn(() => ({ headers: [], rows: [] })),
  formatDescriptiveStatisticsTable: jest.fn(() => ({ headers: [], rows: [] })),
}));

describe('useKIndependentSamplesAnalysis', () => {
  const mockVariables: Variable[] = [
    ...
    },
    {
      ...
      values: [
        ...
        { variableId: 2, value: 'Group2', label: 'Group2' },
        { variableId: 2, value: 'Group3', label: 'Group3' },
        { variableId: 2, value: 'Group3', label: 'Group3' }
      ],
      missing: {},
      align: 'left',
  ...
  const renderTestHook = (props = defaultProps) => {
    return renderHook(() => useKIndependentSamplesAnalysis(props));
  };
  ...
  it('should handle successful analysis completion with Kruskal-Wallis H test', async () => {
    const { result } = renderTestHook();
    ...
    // Simulate k independent samples analysis response
    await act(async () => {
        ...
        mockWorker.onmessage({
          data: {
            ...
            results: {
              ...
              ranks: {
                1: { label: 'Group1', N: 2, MeanRank: 2.5, SumRanks: 5.0 },
                2: { label: 'Group2', N: 2, MeanRank: 4.5, SumRanks: 9.0 },
                3: { label: 'Group3', N: 2, MeanRank: 6.5, SumRanks: 13.0 }
              },
        ...
        mockWorker.onmessage({
          data: {
            ...
            results: {
              ...
              // Missing ranks, testStatisticsKruskalWallisH, etc.
            }
          }
        });
      }
        ...
        mockWorker.onmessage({
          data: {
            ...
            results: {
              ...
              variable2: mockVariables[1],
              ranks: null,
              testStatisticsKruskalWallisH: null,
              metadata: {
  ...
  it('should handle multiple test types simultaneously', async () => {
    ...
    // Simulate response with multiple test types
    await act(async () => {
      if (mockWorker.onmessage) {
        mockWorker.onmessage({
          data: {
        ...
        mockWorker.onmessage({
          data: {
            ...
            results: {
              ...
              variable2: mockVariables[1],
              ranks: {
                1: { label: 'Group1', N: 2, MeanRank: 2.0, SumRanks: 4.0 },
                2: { label: 'Group2', N: 2, MeanRank: 4.0, SumRanks: 8.0 },
                3: { label: 'Group3', N: 2, MeanRank: 6.0, SumRanks: 12.0 }
              },
              testStatisticsKruskalWallisH: {
                H: 4.0,
                df: 2,
          ...
          mockWorker.onmessage({
            data: {
              ...
              results: {
                ...
                variable2: mockVariables[1],
                ranks: {
                  1: { label: 'Group1', N: 6, MeanRank: 3.5, SumRanks: 21.0 }
                },
                testStatisticsKruskalWallisH: {
                  H: 0,
                  df: 0,
