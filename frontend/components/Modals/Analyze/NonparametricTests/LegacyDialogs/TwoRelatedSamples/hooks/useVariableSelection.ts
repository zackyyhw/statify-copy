...
export const useVariableSelection = () => {
    ...
    const getValidationError = (): string | null => {
        if (testVariables1.length === 0 || testVariables2.length === 0) {
            return "Please select at least one pair of variables.";
        }
        
        if (testVariables1.length !== testVariables2.length) {
            return "The number of variables in both lists must be equal.";
        }
        
        // Check for invalid pairs
        for (let i = 0; i < testVariables1.length; i++) {
            if (!isPairValid(i)) {
                return `Pair ${i + 1} is invalid. Variables must be different.`;
            }
        }
