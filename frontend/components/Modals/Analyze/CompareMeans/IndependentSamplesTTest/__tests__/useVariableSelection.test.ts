...
// Tests
// ------------------------------
describe('useVariableSelection hook for IndependentSamplesTTest', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('should move a variable from available to grouping variable', () => {
    const { result } = renderHook(() => useVariableSelection({}));
    ...
    expect(result.current.groupingVariable).not.toBeNull();
    expect(result.current.groupingVariable?.name).toBe('group');
    expect(result.current.availableVariables).toHaveLength(mockVariables.length - 1);
    expect(result.current.testVariables).toHaveLength(0);
  });
  ...
  it('should move a variable back to available variables from grouping variable', () => {
    const { result } = renderHook(() => useVariableSelection({}));

    // Move to grouping variable first
    act(() => {
      result.current.moveToGroupingVariable(mockVariables[2]);
    });
  ...
  it('should move a variable from test to grouping', () => {
    const { result } = renderHook(() => useVariableSelection({}));
    ...
    expect(result.current.testVariables).toHaveLength(0);
    expect(result.current.groupingVariable).not.toBeNull();
    expect(result.current.groupingVariable?.name).toBe('age');
  });

  it('should move a variable from grouping to test', () => {
    const { result } = renderHook(() => useVariableSelection({}));

    // Move to grouping first
    act(() => {
      result.current.moveToGroupingVariable(mockVariables[2]);
    });
  ...
  it('should reset variable selection correctly', () => {
    ...
    // Move variables to test and grouping
    act(() => {
      result.current.moveToTestVariables(mockVariables[0]);
      result.current.moveToGroupingVariable(mockVariables[2]);
    });
