...
const RecodeVariablesTab: FC<RecodeVariablesTabProps> = ({
  ...
  moveToLeftPane,
  reorderVariables = () => {
    console.warn("reorderVariables not implemented upstream");
  },
}) => {
  ...
  const setManagerHighlightedVariable = useCallback(
    (value: { id: string; source: string } | null) => {
      if (
        value &&
        (value.source === "available" || value.source === "recodeList")
      ) {
        // Changed source type
        setHighlightedVariable({
          tempId: value.id,
          source: value.source as "available" | "recodeList",
        });
      } else {
  ...
  const handleMoveVariable = useCallback(
    (
      ...
      if (toListId === "recodeList") {
        // Changed to 'recodeList'
  ...
  const handleReorderVariables = useCallback(
    (listId: string, variables: Variable[]) => {
      if (
        (listId === "recodeList" || listId === "available") &&
        reorderVariables
      ) {
        // Changed to 'recodeList'
