self.onmessage = function(event) {
  ...
  if (!independentData || !coefficients || !dependentData) {
    console.error("[Worker] Missing data:", { independentData, coefficients, dependentData });
    self.postMessage({ success: false, error: "Missing required data." });
    return;
  }

  try {
    console.log("[Worker] Input data:", {
    ...
    const numericCoefficients = coefficients.map((coef, index) => {
      ...
      console.log(`[Worker] Converting coefficient ${index}:`, {
        original: coef,
        numeric: num,
        isNaN: isNaN(num)
      });
    ...
    // 1. Calculate Unstandardized Predicted Values (PRED)
    // Formula: Ŷᵢ = b₀ + Σⱼ₌₁ᵖ bⱼXⱼᵢ
    const unstandardizedPredictions = independentData.map((row, rowIndex) => {
    ...
    // Calculate mean of Y
    const meanY = dependentData.reduce((sum, val) => sum + Number(val), 0) / C;
    
    // Calculate mean of predicted values
    const meanPred = unstandardizedPredictions.reduce((sum, val) => sum + val, 0) / C;

    // Calculate standard deviation of predicted values for ZPRED
    // Formula: sd = √[Σᵢ₌₁ᶜ wᵢ(Ŷᵢ - ȳ)²/(C-1)]
    const sdPred = Math.sqrt(
      unstandardizedPredictions.reduce((sum, pred, i) => {
        const weight = caseWeights[i];
        const diff = pred - meanPred;
        return sum + weight * diff * diff;
      }, 0) / (C - 1)
    );

    // Calculate leverage values using hat matrix diagonal for multiple regression
    ...
    const calculateLeverage = (X) => {
      try {
        ...
        console.log("[Worker] Calculated leverage values:", {
        ...
        console.error("[Worker] Error calculating leverage:", error);
    ...
    // Calculate residuals
    // Formula: eᵢ = Yᵢ - Ŷᵢ
    const residuals = dependentData.map((actual, i) => 
      Number(actual) - unstandardizedPredictions[i]
    );

    // Calculate sum of squares error
    // Formula: SSₑ = Σᵢ₌₁ᶜ wᵢeᵢ²
    const SSe = residuals.reduce((sum, e, i) => 
      sum + caseWeights[i] * e * e, 0);
    
    // Calculate mean square error
    // Formula: MSE = SSₑ/(C-p*)
    const MSE = SSe / (C - p_star);
    ...
    // Calculate deleted residuals (DRESID)
    // Formula: DRESIDᵢ = eᵢ/(1-hᵢ)
    const deletedResiduals = residuals.map((e, i) => {
      const h_i = leverage[i];
      return e / (1 - h_i);
    });

    // Calculate all four types of predicted values
    const results = independentData.map((row, rowIndex) => {
      ...
      // 1. Unstandardized Predicted Values (PRED)
      const unstandardized = unstandardizedPredictions[rowIndex];

      // 2. Standardized Predicted Values (ZPRED)
      // Formula: ZPREDᵢ = (Ŷᵢ - ȳ)/sd
      const standardized = (unstandardized - meanPred) / sdPred;
      
      // 3. Adjusted Predicted Values (ADJPRED)
      // Formula: ADJPREDᵢ = Yᵢ - DRESIDᵢ
      const dresid = deletedResiduals[rowIndex];
      const adjusted = numericDep - dresid;

      // 4. S.E. of mean predictions (SEPRED)
      // Formula: SEPREDᵢ = s√(hᵢ/gᵢ)
      const h_i = leverage[rowIndex];
      const se = s * Math.sqrt(h_i / weight);

      return {
        unstandardized: Number(unstandardized.toFixed(5)),
        standardized: Number(standardized.toFixed(5)),
        adjusted: Number(adjusted.toFixed(5)),
        se: Number(se.toFixed(5))
      };
    });

    console.log("[Worker] Calculation results:", {
      firstRow: results[0],
      standardized_first5: results.map(r => r.standardized).slice(0, 5),
      adjusted_first5: results.map(r => r.adjusted).slice(0, 5),
      se_first5: results.map(r => r.se).slice(0, 5),
      totalRows: results.length,
      MSE: MSE,
      s: s,
      residuals: residuals.slice(0, 5),
      deletedResiduals: deletedResiduals.slice(0, 5),
    ...
    if (results.some(val => 
      ...
      console.error("[Worker] NaN values found in results:", 
        results.filter(val => 
          isNaN(val.unstandardized) || 
          isNaN(val.standardized) || 
          isNaN(val.adjusted) || 
